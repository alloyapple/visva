...
public class Chodao extends ILive{
	...
	public Chodao (GameOS game, IScreen screen) {
		super(game, screen);
	...
	public void start () {
		...
		deltaX = game.X(20);
		xStartAttackLeft = (int)(_screen.nvc.x+_screen.nvc.wi*_screen.nvc.legL+game.X((-180+10)*1f));
	...
	public void draw () {
		...
		if(isFlip)
			...
			game.batcher.draw(Asset.cho_dao.frames[ani.current].region, x, y, Asset.cho_dao.frames[ani.current].wi*1f, Asset.cho_dao.frames[ani.current].hi*1f);
	}
	public void drawFlip()
	{
		game.batcher.draw(Asset.cho_dao.frames[ani.current].region.getTexture(),  x,y,Asset.cho_dao.frames[ani.current].wi*1f, Asset.cho_dao.frames[ani.current].hi*1f,Asset.cho_dao.frames[ani.current].region.getRegionX() , Asset.cho_dao.frames[ani.current].region.getRegionY(), Asset.cho_dao.frames[ani.current].region.getRegionWidth(), Asset.cho_dao.frames[ani.current].region.getRegionHeight(), true, false);
	}
	...
	{		
		...
		comment.set(x+Asset.cho_dao.frames[0].wi*1f/2 - Asset.comment.frames[0].wi/2, y+Asset.cho_dao.frames[0].hi*1f/2-Asset.comment.frames[0].hi/2,score);
