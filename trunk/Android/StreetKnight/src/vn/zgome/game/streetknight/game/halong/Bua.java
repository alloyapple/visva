...
public class Bua extends ILive
{
	int frame = 1;
	int x,y;
	int state;
	...
	public Bua (GameOS game, IScreen screen) {
		super(game, screen);
	...
	public void draw () {
		// TODO Auto-generated method stub
		game.batcher.draw(Asset.bua.frames[frame].region, x, y, Asset.bua.frames[frame].wi, Asset.bua.frames[frame].hi);
		if(state == 2)
		{
	...
	public void update (int delayTime) {
		...
		{
			if(type == 1 || type == 4){
				if((y>=yStartConner&&y<=yEndConner)||(type==4&&x>_screen.nvc.x + _screen.nvc.wi/2 - _screen.nvc.deltaConner +  Asset.bua.frames[0].wi/2)||(type==1&&x<=_screen.nvc.x + _screen.nvc.wi/2 +_screen.nvc.deltaConner -  Asset.bua.frames[0].wi/2))
		...
		tick = 0;
		if(type == 1|| type == 4){
			if((y>=yStartAttackConner&&y<=yEndAttackConner)||((type==4&&(x>=_screen.nvc.x+_screen.nvc.wi/2-Asset.bua.frames[0].wi/2)||(type==1&&(x>=xStartRight&&x<=_screen.nvc.x+_screen.nvc.wi/2-Asset.bua.frames[0].wi/2)))))
		{	
			state = 2;
	...
	{		
		...
		comment.set(x+Asset.bua.frames[0].wi/2 - Asset.comment.frames[0].wi/2, y+Asset.bua.frames[0].hi/2-Asset.comment.frames[0].hi/2,score);
