...
public class Riu extends ILive
{
	int frame = 1;
	int x,y;
	int state;
	...
	public Riu (GameOS game, IScreen screen) {
		super(game, screen);
	...
	public void draw () {
		// TODO Auto-generated method stub
		game.batcher.draw(Asset.riu.frames[frame].region, x, y, Asset.riu.frames[frame].wi, Asset.riu.frames[frame].hi);
		if(state == 2)
		{
	...
	public void update (int delayTime) {
		...
		{
			if(type == 1 || type == 4){
				if((y>=yStartConner&&y<=yEndConner)||(type==4&&x>_screen.nvc.x + _screen.nvc.wi/2 - _screen.nvc.deltaConner +  Asset.riu.frames[0].wi/2)||(type==1&&x<=_screen.nvc.x + _screen.nvc.wi/2 +_screen.nvc.deltaConner -  Asset.riu.frames[0].wi/2))
		...
		tick = 0;
		if(type == 1|| type == 4){
			if((y>=yStartAttackConner&&y<=yEndAttackConner)||((type==4&&(x>=_screen.nvc.x+_screen.nvc.wi/2-Asset.riu.frames[0].wi/2)||(type==1&&(x>=xStartRight&&x<=_screen.nvc.x+_screen.nvc.wi/2-Asset.riu.frames[0].wi/2)))))
		{	
			state = 2;
	...
	{
		x = x+Asset.riu.frames[0].wi/2;
		y = y+Asset.riu.frames[0].hi/2;
		
		x = x-Asset.riu.frames[9].wi/2;
		y = y-Asset.riu.frames[9].hi/2;
		
		xBlood3 = x + Asset.riu.frames[9].wi/2;
		yBlood3 = y + Asset.riu.frames[9].wi/2;
	...
	{		
		...
		comment.set(x+Asset.riu.frames[0].wi/2 - Asset.comment.frames[0].wi/2, y+Asset.riu.frames[0].hi/2-Asset.comment.frames[0].hi/2,score);
