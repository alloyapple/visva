...
public class Bom extends ILive
{
	int frame = 1;
	int x,y;
	int state;
	...
	public Bom (GameOS game, IScreen screen) {
		super(game, screen);
	...
	public void draw () {
		// TODO Auto-generated method stub
		game.batcher.draw(Asset.bom.frames[frame].region, x, y, Asset.bom.frames[frame].wi, Asset.bom.frames[frame].hi);
	}
	int tick;
	@Override
	public void update (int delayTime) {
		...
		{
			if(type == 1 || type == 4){
				if((y>=yStartConner&&y<=yEndConner)||(type==4&&x>_screen.nvc.x + _screen.nvc.wi/2 - _screen.nvc.deltaConner -  Asset.bom.frames[0].wi/(2))||(type==1&&x<=_screen.nvc.x + _screen.nvc.wi/2 +_screen.nvc.deltaConner))
			{				
				state = 2;
				ani.setInfo(frameStartKick, frameEndKick, timeFrameKick);
				vibration(false);
		...
		tick = 0;
		if(type == 1|| type == 4){
			if((y>=yStartAttackConner&&y<=yEndAttackConner)||((type==4&&(x<=xEndLeft&&x>=_screen.nvc.x+_screen.nvc.wi/2-Asset.bom.frames[0].wi/2)||(type==1&&(x>=xStartRight&&x<=_screen.nvc.x+_screen.nvc.wi/2)))))
	...
	{		
		...
		comment.set(x+Asset.bom.frames[0].wi/2 - Asset.comment.frames[0].wi/2, y+Asset.bom.frames[0].hi/2-Asset.comment.frames[0].hi/2,score);
