package vn.zgome.game.streetknight.game.japan;
...
public class Phitieu extends ILive
{
	int frame = 1;
	int x,y;
	int state;
	...
	public Phitieu (GameOS game, IScreen screen) {
		super(game, screen);
	...
	public void start () {
		...
		x = game.X(839-200/2);
		y = game.Y(490-150/2);
		xCPy = ((float)((x+game.X(200/2))-(_screen.nvc.x+_screen.nvc.xHead))/(float)((y+game.Y(150/2))-(_screen.nvc.y+_screen.nvc.yHead)));
		deltaX=game.X(9);
		...
		xStartLeft = (int)(_screen.nvc.x-game.X(200));
		...
		xStartAttackLeft = (int)(_screen.nvc.x + _screen.nvc.wi/2 + game.X((75-150)));
	...
	public void draw () {
		// TODO Auto-generated method stub
		game.batcher.draw(Asset.phi_tieu.frames[frame].region, x, y, Asset.phi_tieu.frames[frame].wi, Asset.phi_tieu.frames[frame].hi);
		if(state == 2)
		{
	...
	public void update (int delayTime) {
		...
		{
			if(type == 1 || type == 4){
				if((y>=yStartConner&&y<=yEndConner)||(type==4&&x>_screen.nvc.x + _screen.nvc.wi/2 - _screen.nvc.deltaConner +  Asset.phi_tieu.frames[0].wi/(2))||(type==1&&x<=_screen.nvc.x + _screen.nvc.wi/2 +_screen.nvc.deltaConner -  Asset.phi_tieu.frames[0].wi/(2)))
			{				
				state = 2;
				ani.setInfo(frameStartKick, frameEndKick, timeFrameKick);
		...
		tick = 0;
		if(type == 1|| type == 4){
			if((y>=yStartAttackConner&&y<=yEndAttackConner)||((type==4&&(x>=_screen.nvc.x+_screen.nvc.wi/2-Asset.phi_tieu.frames[0].wi/2)||(type==1&&(x<=_screen.nvc.x+_screen.nvc.wi/2-Asset.phi_tieu.frames[0].wi/2)))))
		{	
			state = 2;
	...
	{
		x = x+Asset.phi_tieu.frames[0].wi/2;
		y = y+Asset.phi_tieu.frames[0].hi/2;
		
		x = x-Asset.phi_tieu.frames[9].wi/2;
		y = y-Asset.phi_tieu.frames[9].hi/2;
		
		xBlood3 = x + Asset.phi_tieu.frames[9].wi/2;
		yBlood3 = y + Asset.phi_tieu.frames[9].wi/2;
	...
	{		
		...
		comment.set(x+Asset.phi_tieu.frames[0].wi/2 - Asset.comment.frames[0].wi/2, y+Asset.phi_tieu.frames[0].hi/2-Asset.comment.frames[0].hi/2,score);
