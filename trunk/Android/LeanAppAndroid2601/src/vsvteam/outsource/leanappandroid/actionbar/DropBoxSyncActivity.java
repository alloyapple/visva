...
public class DropBoxSyncActivity extends Activity {
	...
	// Obfuscation is good.
	final static private String APP_KEY = "ywas0cphzpvdzej";
	final static private String APP_SECRET = "4j62eph1mlywcy4";
	...
	private PDFDocumentController pdfDocument;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		/* get format file to export */
		isSendPdfFile = getIntent().getExtras().getBoolean("isSendFileFormat");
		Log.e("isSendFileFormat", "isSendFileFormat "+isSendPdfFile);
	...
	/**
	 * create pdf file in sdcard
	 * 
	 * @param
	 */
	private void createPdfFile(String filePdfName) {
		try {
			pdfDocument = new PDFDocumentController(DropBoxSyncActivity.this);
			Document document = pdfDocument
					.creatPDFDocument(PATH_FOLDER_DOCUMENT, "" + filePdfName);
			document.open();
			// add text
			pdfDocument.addText(document, "First Table");

			// add header table
			String[] header = { "Column1", "Column2", "Column3", "column 4" };
			PdfPTable myTable = pdfDocument.addTable(header);

			// add content table
			String[] tableContent = { "1", "2", "3", "4" };
			pdfDocument.addContentTable(myTable, tableContent);
		...
		/* export file pdf to dropbox */
		exportFilePdfToDropBox(filePdfName);
	}
