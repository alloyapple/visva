...
import com.visva.soundanalyzer.VoiceChunk;
...
public class CheapWAV extends CheapSoundFile {
    ...
    public void WriteFileModified(File outputFile, int startFrame, int numFrames,
    		double ratio, ArrayList<VoiceChunk> voices)
        ...
        for (int i = 0; i < numFrames; i++) {
            ...
            for(int j=0;j<voices.size();j++){
            	VoiceChunk voice = (VoiceChunk)voices.get(j);
            	if( i >=voice.start && i <= voice.end && voice.needToIncrease ){
            		isValid = true;
            		break;
            	}
            }
            if( isValid ){
            	for (int j = 0; j < mFrameBytes; j += 4) {
                    int val = ((0xff & buffer[j+3]) << 24) |
                            	((0xff & buffer[j+2]) << 16) |
                            	((0xff & buffer[j+1]) << 8) |
                            	((0xff & buffer[j]));
                    val *= ratio;
                    ...
                    buffer[j+1] = (byte) ((val >> 8) & 0xff);
                    buffer[j+2] = (byte) ((val >> 16) & 0xff);
                    buffer[j+3] = (byte) ((val >> 24) & 0xff);
                }
            }	
