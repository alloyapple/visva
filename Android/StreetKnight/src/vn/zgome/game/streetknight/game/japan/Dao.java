...
public class Dao extends ILive
{
	int frame = 1;
	int x,y;
	int state;
	...
	public Dao (GameOS game, IScreen screen) {
		super(game, screen);
	...
	public void start () {
		...
		x = game.X(839-150/2);
		y = game.Y(490-150/2);
		xCPy = ((float)((x+game.X(150/2))-(_screen.nvc.x+_screen.nvc.xHead))/(float)((y+game.Y(150/2))-(_screen.nvc.y+_screen.nvc.yHead)));
		deltaX=game.X(9);
		...
		xStartLeft = (int)(_screen.nvc.x-game.X(150));
	...
	public void draw () {
		// TODO Auto-generated method stub
		game.batcher.draw(Asset.dao.frames[frame].region, x, y, Asset.dao.frames[frame].wi, Asset.dao.frames[frame].hi);
		if(state == 2)
		{
	...
	public void update (int delayTime) {
		...
		{
			if(type == 1 || type == 4){
				if((y>=yStartConner&&y<=yEndConner)||(type==4&&x>_screen.nvc.x + _screen.nvc.wi/2 - _screen.nvc.deltaConner +  Asset.dao.frames[0].wi/2)||(type==1&&x<=_screen.nvc.x + _screen.nvc.wi/2 +_screen.nvc.deltaConner -  Asset.dao.frames[0].wi/2))
			{				
				state = 2;
				ani.setInfo(frameStartKick, frameEndKick, timeFrameKick);
		...
		tick = 0;
		if(type == 1|| type == 4){
			if((y>=yStartAttackConner&&y<=yEndAttackConner)||((type==4&&(x>=_screen.nvc.x+_screen.nvc.wi/2-Asset.dao.frames[0].wi/2)||(type==1&&(x<=_screen.nvc.x+_screen.nvc.wi/2-Asset.dao.frames[0].wi/2)))))
		{	
			state = 2;
	...
	{
		x = x+Asset.dao.frames[0].wi/2;
		y = y+Asset.dao.frames[0].hi/2;
		
		x = x-Asset.dao.frames[9].wi/2;
		y = y-Asset.dao.frames[9].hi/2;
		
		xBlood3 = x + Asset.dao.frames[9].wi/2;
		yBlood3 = y + Asset.dao.frames[9].wi/2;
	...
	{		
		...
		comment.set(x+Asset.dao.frames[0].wi/2 - Asset.comment.frames[0].wi/2, y+Asset.dao.frames[0].hi/2-Asset.comment.frames[0].hi/2,score);
