...
public class Caikim extends ILive
{
	int frame = 1;
	int x,y;
	int state;
	...
	public Caikim (GameOS game, IScreen screen) {
		super(game, screen);
	...
	public void start () {
		...
		if(type == 1){
		x = (int)game.X(839-300*0.45f);
		y = (int)game.Y(490-250*0.45f);
		xCPy = ((float)((x+game.X(300*0.45f/2))-(_screen.nvc.x+_screen.nvc.xHead))/(float)((y+game.Y(250*0.45f/2))-(_screen.nvc.y+_screen.nvc.yHead)));
		deltaX=game.X(9);
		deltaY=(int)game.Y(9/xCPy);
		}
		else if(type==4)
		{
			x = (int)game.X(0-300*0.45f);
			y = (int)game.Y(490-250*0.45f);
			xCPy = ((float)(-(x+game.X(300*0.45f/2))+(_screen.nvc.x+_screen.nvc.xHead))/(float)((y+game.Y(250*0.45f/2))-(_screen.nvc.y+_screen.nvc.yHead)));
			deltaX=game.X(9);
			deltaY=(int)game.Y(9/xCPy);
		...
		xStartLeft = (int)(_screen.nvc.x-game.X(300*0.45f));
		...
		xStartAttackLeft = (int)(_screen.nvc.x + _screen.nvc.wi/2 + game.X((150-300)*0.45f));
	...
	public void draw () {
		// TODO Auto-generated method stub
		game.batcher.draw(Asset.cai_kim.frames[frame].region, x, y, Asset.cai_kim.frames[frame].wi*0.45f, Asset.cai_kim.frames[frame].hi*0.45f);
		if(state == 2)
		{
	...
	public void update (int delayTime) {
		...
		{
			if(type == 1 || type == 4){
				if((y>=yStartConner&&y<=yEndConner)||(type==4&&x>_screen.nvc.x + _screen.nvc.wi/2 - _screen.nvc.deltaConner + Asset.cai_kim.frames[0].wi*0.45f/2)||(type==1&&x<=_screen.nvc.x + _screen.nvc.wi/2 +_screen.nvc.deltaConner -  Asset.cai_kim.frames[0].wi*0.45f/2))
			{				
				state = 2;
				ani.setInfo(frameStartKick, frameEndKick, timeFrameKick);
		...
		tick = 0;
		if(type == 1|| type == 4){
			if((y>=yStartAttackConner&&y<=yEndAttackConner)||((type==4&&(x>=_screen.nvc.x+_screen.nvc.wi/2-Asset.cai_kim.frames[0].wi*0.45f/2)||(type==1&&(x<=_screen.nvc.x+_screen.nvc.wi/2-Asset.cai_kim.frames[0].wi*0.45f/2)))))
		{	
			state = 2;
	...
	{
		x = (int)(x+Asset.cai_kim.frames[0].wi*0.35f/2);
		y = (int)(y+Asset.cai_kim.frames[0].hi*0.35f/2);
		
		x = (int)(x-Asset.cai_kim.frames[9].wi*0.35f/2);
		y = (int)(y-Asset.cai_kim.frames[9].hi*0.35f/2);
		
		xBlood3 = (int)(x + Asset.cai_kim.frames[9].wi*0.35f/2);
		yBlood3 = (int)(y + Asset.cai_kim.frames[9].wi*0.35f/2);
	...
	{		
		...
		comment.set(x+Asset.cai_kim.frames[0].wi*0.45f/2 - Asset.comment.frames[0].wi/2, y+Asset.cai_kim.frames[0].hi*0.45f/2-Asset.comment.frames[0].hi/2,score);
