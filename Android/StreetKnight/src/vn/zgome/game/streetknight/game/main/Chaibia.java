...
public class Chaibia extends ILive
{
	int frame = 1;
	int x,y;
	int state;
	...
	public Chaibia (GameOS game, IScreen screen) {
		super(game, screen);
	...
	public void start () {
		...
		xStartLeft = (int)(_screen.nvc.x-game.X(80*0.9f));
	...
	public void draw () {
		...
		game.batcher.draw(Asset.chai_bia.frames[frame].region, x, y, Asset.chai_bia.frames[frame].wi * scale, Asset.chai_bia.frames[frame].hi * scale);
	...
	public void update (int delayTime) {
		...
		{
			if(type == 1 || type == 4){
				if((y>=yStartConner&&y<=yEndConner)||(type==4&&x>_screen.nvc.x + _screen.nvc.wi/2 - _screen.nvc.deltaConner)||(type==1&&x<=_screen.nvc.x + _screen.nvc.wi/2 +_screen.nvc.deltaConner -  Asset.chai_bia.frames[0].wi*0.9f/2))
			{				
				state = 2;
				ani.setInfo(frameStartKick, frameEndKick, timeFrameKick);
				...
				brokenXY();
				Asset.glass_break.play(game.dataSave.sound?1f:0f);
			}
			}
			else if(type == 2)
			{
		...
		tick = 0;
		if(type == 1|| type == 4){
		if((y>=yStartAttackConner&&y<=yEndAttackConner)||((type==4&&(x>=_screen.nvc.x+_screen.nvc.wi/2-Asset.chai_bia.frames[0].wi*0.9f/2)||(type==1&&(x<=_screen.nvc.x+_screen.nvc.wi/2-Asset.chai_bia.frames[0].wi*0.9f/2)))))
		{	
			state = 2;
	...
	{
		x = x+Asset.chai_bia.frames[0].wi/2;
		y = y+Asset.chai_bia.frames[0].hi/2;
		
		x = x-Asset.chai_bia.frames[9].wi/2;
		y = y-Asset.chai_bia.frames[9].hi/2;
		
		xBlood3 = x + Asset.chai_bia.frames[9].wi/2;
		yBlood3 = y + Asset.chai_bia.frames[9].wi/2;
	...
	{		
		...
		comment.set(x+Asset.chai_bia.frames[0].wi/2 - Asset.comment.frames[0].wi/2, y+Asset.chai_bia.frames[0].hi/2-Asset.comment.frames[0].hi/2,score);
