...
public class MoNeo extends ILive
{
	int frame = 1;
	int x,y;
	int state;
	...
	public MoNeo (GameOS game, IScreen screen) {
		super(game, screen);
	...
	public void draw () {
		// TODO Auto-generated method stub
		game.batcher.draw(Asset.mo_neo.frames[frame].region, x, y, Asset.mo_neo.frames[frame].wi, Asset.mo_neo.frames[frame].hi);
		if(state == 2)
		{
	...
	public void update (int delayTime) {
		...
		{
			if(type == 1 || type == 4){
				if((y>=yStartConner&&y<=yEndConner)||(type==4&&x>_screen.nvc.x + _screen.nvc.wi/2 - _screen.nvc.deltaConner +  Asset.mo_neo.frames[0].wi/2)||(type==1&&x<=_screen.nvc.x + _screen.nvc.wi/2 +_screen.nvc.deltaConner -  Asset.mo_neo.frames[0].wi/2))
				{				
				state = 2;
				ani.setInfo(frameStartKick, frameEndKick, timeFrameKick);
		...
		tick = 0;
		if(type == 1|| type == 4){
			if((y>=yStartAttackConner&&y<=yEndAttackConner)||((type==4&&(x>=_screen.nvc.x+_screen.nvc.wi/2-Asset.mo_neo.frames[0].wi/2)||(type==1&&(x<=_screen.nvc.x+_screen.nvc.wi/2-Asset.mo_neo.frames[0].wi/2)))))
		{	
			state = 2;
	...
	{
		x = x+Asset.mo_neo.frames[0].wi/2;
		y = y+Asset.mo_neo.frames[0].hi/2;
		
		x = x-Asset.mo_neo.frames[9].wi/2;
		y = y-Asset.mo_neo.frames[9].hi/2;
		
		xBlood3 = x + Asset.mo_neo.frames[9].wi/2;
		yBlood3 = y + Asset.mo_neo.frames[9].wi/2;
	...
	{		
		...
		comment.set(x+Asset.mo_neo.frames[0].wi/2 - Asset.comment.frames[0].wi/2, y+Asset.mo_neo.frames[0].hi/2-Asset.comment.frames[0].hi/2,score);
