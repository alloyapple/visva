...
public class Facebook {
	 ...
	 */
	public Facebook(String appId, MainScreenActivity mainScreenActivity) {
		if (appId == null) {
		...
		mSpinner = new ProgressDialog(mainScreenActivity.getBaseContext());
	...
	private void startDialogAuth(Activity activity, String[] permissions) {
		...
		dialog(activity, LOGIN, params, new DialogListener() {
			...
			public void onComplete(Bundle values) {
				...
				if (isSessionValid()) {
					// UtilityFacebook.mAsyncRunner.request("me", new
					// SampleRequestListener());
					mAuthDialogListener.onComplete(values);
					Log.e(TAG, "session ok");
				} else {
					// mainScreenActivity.toggleButtonShareFacebook.setChecked(false);
					Log.e(TAG, "session no ok");
			...
			public void onError(DialogError error) {
				Log.d("Facebook-authorize", "Login failed: (1) " + error);
				mAuthDialogListener.onError(error);
				mainScreenActivity.toggleButtonShareFacebook.setChecked(false);
			}

			public void onFacebookError(FacebookError error) {
				Log.d("Facebook-authorize", "Login failed: (2) " + error);
	 ...
	 */
	public void authorizeCallback(MainScreenActivity mainScreenActivity, int requestCode,
			...
			if (resultCode == Activity.RESULT_OK) {
				...
				if (error != null) {
					if (error.equals(SINGLE_SIGN_ON_DISABLED)
						...
						Log.d("Facebook-authorize", "Login failed: (3) " + error);
				...
				if (data != null) {
					Log.d("Facebook-authorize", "Login failed: (4) " + data.getStringExtra("error"));
	...
	public void dialog(Context context, String action, Bundle parameters,
		...
		if (context.checkCallingOrSelfPermission(Manifest.permission.INTERNET) != PackageManager.PERMISSION_GRANTED) {
			...
			Log.e("Time process", "" + (end - start));
			new FbDialog(context, url, listener).show();
			// new ClearCookie(context, url, listener).execute();
		}
	}
	...
	public void setAccessToken(String token) {
		mAccessToken = token;
		mainScreenActivity.ujooltSharedPreferences.putAccessTokenFacebookUserName(token);
	...
	// Log.e("vao oncomplete", "ok");
	// mainScreenActivity.isLoading = true;
	// try {
	// // process the response here: executed in background thread
	// final JSONObject json = Util.parseJson(response);
	// mainScreenActivity.myUserNameFacebook = json.getString("name");
	// mainScreenActivity.myUserIdFacebook = json.getString("id");
	// mainScreenActivity.isFacebookConnected = true;
	// mainScreenActivity.closeProgressDialog();
	//
	// mainScreenActivity.runOnUiThread(new Runnable() {
	// public void run() {
	//
	// if (mainScreenActivity.viewLogin.isLogin) {
	// mainScreenActivity.myUserName = mainScreenActivity.myUserNameFacebook;
	// mainScreenActivity.myLoginType = "FB";
	// mainScreenActivity.myLoginUserid = mainScreenActivity.myUserIdFacebook;
	...
	class ClearCookie extends AsyncTask<String, String, String> {
		...
		public ClearCookie(Context context, String url, DialogListener dialogListener) {
			...
			progressDialog.setMessage(Language.loading);
